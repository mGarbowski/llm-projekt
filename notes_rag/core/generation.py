...
class Generator:
    ...
    def build_prompt(self, user_prompt: str, context: list[NoteChunk], message_history: list[dict[str, str]]):
        context_text = (
            "\n\n".join([f"[{i + 1}] {d.title}, {d.course}\n{d.content}" for i, d in enumerate(context)])
            if context
            else ""
        )
        ...
        messages = [
            ...
            *message_history,
            {"role": "assistant", "content": f"Źródła do kolejnej odpowiedzi:\n{context_text}"},
